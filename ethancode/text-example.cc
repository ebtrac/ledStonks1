...
int main(int argc, char *argv[]) {
  ...
  matrix_options.pixel_mapper_config = "U-mapper;Rotate:270";
  matrix_options.limit_refresh_rate_hz = 122; //fps cap
  matrix_options.show_refresh_rate = true; //for debugging and tweaking
  ...
  const char *bdf_font_file = "/home/dietpi/ledmatrix/fonts/4x6.bdf"; //was NULL
  //just fyi, this font gives us 16 columns and 10 rows.
  int x_orig = 0;
  int y_orig = 0;
  ...
  //add a file here
  std::ifstream inf{ "/home/dietpi/ledmatrix/ethancode/ethan.cc" };

  canvas->Fill(flood_color.r, flood_color.g, flood_color.b);
  char line[1024];
  while (inf) { 
    std::string str_line; //string object for reading a line from the file
    std::getline(inf, str_line); //read a line from input file into str_line
    std::strcpy(line, str_line.c_str()); //convert str_line to char array and store to line
